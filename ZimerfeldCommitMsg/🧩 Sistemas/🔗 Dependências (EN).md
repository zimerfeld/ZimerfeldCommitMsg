...
> of GitExtensions 7.x advertises `7.0.0`, so the range must **contain** 7.0.0 → `[7.0.0, 8.0.0)`.
> The plugin is compiled against **Extensibility 7.2** (net10): GE7 changed the `AddCommitTemplate`/`StartCommitDialog`
> signatures, so a DLL compiled for GE6 made the templates **silently disappear** in the 7.x host's dropdown.
> A loose value such as `1.0.0.129` means `>= 1.0.0.129`, which does **not** include 7.0.0,
> and the package would be **silently filtered out** of the Plugin Manager. To go back to **GitExtensions 6**
> (the v3.x manager advertises `0.4.0`), use `[0.4.0, 0.5.0)` and Extensibility 6.x. Aligned with [[GitExtensions.ZimerfeldTree]].
...
- The same DLL also in `tools\net10.0-windows\` for installing via **Package Manager Console** (`install.ps1`).
