...
> v7.x de GitExtensions 7.x anuncia `7.0.0`, así que el rango debe **contener** 7.0.0 → `[7.0.0, 8.0.0)`.
> El plugin se compila contra **Extensibility 7.2** (net10): GE7 cambió las firmas de
> `AddCommitTemplate`/`StartCommitDialog`, así que una DLL compilada para GE6 hacía que las plantillas **desaparecieran**
> en el dropdown del host 7.x. Un valor suelto como `1.0.0.129` significa `>= 1.0.0.129`, que **no** incluye 7.0.0,
> y el paquete se **filtraría silenciosamente hacia fuera** del Plugin Manager. Para volver a **GitExtensions 6**
> (el manager v3.x anuncia `0.4.0`), usar `[0.4.0, 0.5.0)` y la Extensibility 6.x. Alineado con [[GitExtensions.ZimerfeldTree]].
...
- La misma DLL también en `tools\net10.0-windows\` para la instalación vía **Package Manager Console** (`install.ps1`).
